...
StatusType streaming_database::add_movie(int movieId , Genre genre , int views , bool vipOnly){
    ...
    Node<std::shared_ptr<Movie>,int>* to_add_none_rating = nullptr;
    Node<std::shared_ptr<Movie>,int>* to_add_genre= nullptr;

    try {
        new_movie = std::shared_ptr<Movie>(new Movie(movieId, genre, views, vipOnly));
        to_add_none_id = new Node<int, Movie>(movieId, new_movie);
        to_add_none_rating = new Node<std::shared_ptr<Movie>,int>(new_movie, std::make_shared<int>(movieId));
        to_add_genre = new Node<std::shared_ptr<Movie>,int>(new_movie, std::make_shared<int>(movieId));
    }
    catch(...){
        return StatusType::ALLOCATION_ERROR;
    }

    if(new_movie->check_movie() == false) return StatusType::INVALID_INPUT;
    if(none_tree_by_id.add_node(to_add_none_id) == false) return StatusType::FAILURE;
...
StatusType streaming_database::remove_movie(int movieId){
    ...
    std::shared_ptr<Movie> newKey = none_tree_by_id.find_node(movieId)->get_data();
...
StatusType streaming_database::add_user(int userId, bool isVip) {
    ...
    if(users_tree.add_node(to_add) == false) return StatusType::FAILURE;
...
StatusType streaming_database::remove_user(int userId){
    ...
    if(groupId != 0){
        group_tree.find_node(groupId)->get_data()->DeleteUserFromGroup(userId);
    }
...
StatusType streaming_database::remove_group(int groupId){
    if(groupId <= 0) return StatusType::INVALID_INPUT;
    Node<int,Group>* curr_group = empty_groups_tree.find_node(groupId);
    if(curr_group != nullptr){
        empty_groups_tree.Delete(groupId);
        return StatusType::SUCCESS;
    }
    curr_group = group_tree.find_node(groupId);
    if (curr_group == nullptr) return StatusType::FAILURE;
...
StatusType streaming_database::add_user_to_group(int userId, int groupId) {
    ...
    if( userToFind == nullptr || (groupToFindFull == nullptr && groupToFindInEmpty == nullptr) || userToFind->get_data()->get_group_id() != 0 ){
        return StatusType::FAILURE;
    }
...
StatusType streaming_database::user_watch(int userId, int movieId){
    ...
    none_tree_by_rating.removeThenInsert(data , key);
    if(movie_genre == Genre::COMEDY){
        comedy_tree.removeThenInsert(data , key);
    }
    else if(movie_genre == Genre::DRAMA){
        drama_tree.removeThenInsert(data , key);
    }
    else if(movie_genre == Genre::FANTASY){
        fantasy_tree.removeThenInsert(data, key);
    }
    else if(movie_genre == Genre::ACTION){
        action_tree.removeThenInsert(data, key);
    }
    return StatusType::SUCCESS;
}
...
StatusType streaming_database::group_watch(int groupId, int movieId){
    ...
    int* groupViewsArray = groupToFind->get_data()->get_views_array();
    int* groupViewsArrayTotal = groupToFind->get_data()->get_total_views_array();
...
StatusType streaming_database::get_all_movies(Genre genre, int* const output){
    ...
    getIdOfNode<std::shared_ptr<Movie>, int> getIdFunctor(output);

    if(genre == Genre::NONE){
        none_tree_by_rating.RL_inOrder<getIdOfNode<std::shared_ptr<Movie>, int>>(getIdFunctor ,none_tree_by_rating.get_root());
    }
    else if(genre == Genre::COMEDY){
        comedy_tree.RL_inOrder<getIdOfNode<std::shared_ptr<Movie>, int>>(getIdFunctor ,comedy_tree.get_root());
    }
    else if(genre == Genre::ACTION){
        action_tree.RL_inOrder<getIdOfNode<std::shared_ptr<Movie>, int>>(getIdFunctor ,action_tree.get_root());
    }
    else if(genre == Genre::FANTASY){
        fantasy_tree.RL_inOrder<getIdOfNode<std::shared_ptr<Movie>, int>>(getIdFunctor ,fantasy_tree.get_root());
    }
    else if(genre == Genre::DRAMA){
        drama_tree.RL_inOrder<getIdOfNode<std::shared_ptr<Movie>, int>>(getIdFunctor ,drama_tree.get_root());
    }
    return StatusType::SUCCESS;
}
